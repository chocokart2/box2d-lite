...
typedef map<OurArbiterKey, OurArbiter>::Ouriterator OurArbiter;
typedef pair<OurArbiterKey, OurArbiter> ArbPair;
...
void OurWorld::BroadPhase()
{
	...
	for (int i = 0; i < (int)bodies.size(); ++i)
	{
		...
		for (int j = i + 1; j < (int)bodies.size(); ++j)
		{
			...
			{
				OurArbiter iter = arbiters.find(key);
...
void OurWorld::Step(float dt)
{
	...
	// Perform pre-steps.
	for (OurArbiter arb = arbiters.begin(); arb != arbiters.end(); ++arb)
	{
